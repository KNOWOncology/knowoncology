...
const SummarySchema = new mongoose.Schema({
  ...
  studyDesignType: {  
    ...
    entry: {
      type: String,
      required: true,
      enum: ['Systematic Review and/or Meta Analysis', 'Observational Study', 'Clinical Trial', 'Case Report/Series']
    } 
  },
  publicSummary: {  
    tag: {
...
SummarySchema.statics.findegg = async function() {
...
SummarySchema.statics.findWithQuery = function(searchObject) {
  ...
  const searchTerms = searchObject.searchTextInput;
  const searchFilters = searchObject.selectedOptionsArray;

  // return all results if no search parameters
  if(!searchObject ||
    (searchFilters.length === 0 && searchTerms === ''))
    return this.find();  if(searchTerms === '*') return this.findegg();
  
  // ['Stage:I', 'studyYear:2019', 'Stage:II', ...]
  const userFilters = {};

  // parse the searchObject array
  searchFilters.map(selection => selection.split(':'))
...
SummarySchema.pre('validate', function(next) {
  this.yearString = this.yearPublished.entry.toString();
});

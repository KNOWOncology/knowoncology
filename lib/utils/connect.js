...
module.exports = (url = process.env.MONGODB_URI) => {
  ...
  mongoose.connection.on('disconnected', () => console.log('Disconnected from MongoDB')); // eslint-disable-line no-console

  mongoose.connection.on('error', () => console.log('Error connecting to MongoDB')); // eslint-disable-line no-console
};
